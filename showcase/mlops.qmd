...
## The stack: deterministic orchestration, Langfuse

Two pieces, one job each, wired so a turn is inspectable end to end.

- **The omni orchestrator** is deterministic Python. It routes each turn through a cheap-first 3-layer cascade (`rag/router.py`), then answers through the one gated pipeline (`pipeline/answer.py`). The LLM is called only at the decision points (an ambiguous-route tie-break, then generation), not inside a framework loop. This is Anthropic's routing plus orchestrator-worker pattern and 12-Factor Agents' "own your control flow". An earlier LangGraph supervisor brain was retired because a second control-flow framework was a second safety surface to keep in parity, with no measured win over the deterministic path.
- **Langfuse** is the observability: one root span per turn (`@observe`), every LLM generation nested under it with model, tokens, latency, and cost. The LLM is a custom Groq adapter, so tracing uses spans directly. One turn maps to one trace in the Langfuse UI.
...
Operating cost is a first-class number, not an afterthought, and it is the number the business actually reads. A reproducible cost model (`mlops/cost_model.py`) puts each option side by side for a typical **8-turn conversation**:
