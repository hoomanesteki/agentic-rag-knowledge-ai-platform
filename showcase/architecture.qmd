...
Picture the assistant as a small team behind one chat box. A **manager** reads every message and passes it to whoever is best placed to help, and the specialists all answer through the same safety checks, so no one can cut a corner.

::: {.grid-cards}
::: {.card}
### 🧭 The orchestrator
The manager. Reads each turn, decides who should handle it, splits a two-in-one request, and asks a clarifying question rather than guessing. Costs nothing on the easy majority.
:::
::: {.card}
### 🛍️ Sara · shopping
