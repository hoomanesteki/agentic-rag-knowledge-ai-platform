...
- **If the domain becomes regulated (legal, health, finance).** The PII masking, the read-only governed queries, and the deterministic privacy guards are already the spine of a compliance story. The additions are an audit log on every disclosure, data-retention policy on the trace store, and a human-review requirement wired into the existing escalation queue. Nothing needs to be torn out to add them.
