...
"List your customers / who bought X" refuses before retrieval, so no customer's name is ever volunteered.
:::
::: {.card}
### Gender correctness
...
- **Continuous training is scheduled; promotion stays manual.** The drift monitors, MLflow logging, RAGAS eval, and the prompt-optimization loop are real and reproducible, and the CT workflow (`.github/workflows/ct.yml`) now runs the drift check, retrain, and eval gate weekly rather than only on demand. What stays deliberately manual is the deploy: CT proposes a promotion as an artifact and a human approves it, so nothing self-ships.
