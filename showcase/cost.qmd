...
::: {.eyebrow}
Every number here is computed, not typed. This is the proof behind "430× cheaper than a human."
:::

The headline stat, **a session costs about 430× less than a human agent handles it**, is not a marketing figure. It is the output of a small, transparent cost model (`mlops/cost_model.py`) that anyone can read, change, and rerun:

```bash
PYTHONPATH=. uv run python -m mlops.cost_model   # regenerates every figure on this page
```

If a stakeholder asks *"where does 430× come from?"*, the answer below is a chain of tables, not a claim: the assumptions, the price of each stage of a turn, the turn total, the session total, and the division.

## The assumptions (change any of them and rerun)
