...
`make check` is the honest signal: it runs the linter, 395 tests, the domain-leak check, the dbt semantic-layer build, and the eval gate, all with no external services. If it's green, the machinery works. `make doctor` explains what's missing if a step hangs (Docker, `.env`, or a provider key).
