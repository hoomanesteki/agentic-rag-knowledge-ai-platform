...
Every request is first routed to a specialized lane by a master orchestrator, then it follows the same path: understand, retrieve, ground, and either answer with citations or abstain. The engine that runs this path is domain-agnostic, it reads a domain pack (a folder of data and a manifest) and nothing about the store is compiled in.

## The master orchestrator

Every turn goes first to a master orchestrator that decides which lane should handle it, then answers through the one gated pipeline below. Routing is a cheap-first cascade, so the common case costs nothing and only genuine ambiguity pays for a model call.
