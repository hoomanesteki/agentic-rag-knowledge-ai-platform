...
Every choice here is a trade-off against constraints: it runs on a lean budget, it has to work offline with no API keys, and it has to be swappable so it isn't wedded to one vendor. Below is the reasoning, and the honest "if your constraints differ, switch to this" for each.
...
## Two brains, one seam (and one we retired)

**Chosen:** a master orchestrator (the default) and a fast linear pipeline, selectable by one `CHAT_BRAIN` flag.

The default `omni` brain is the master orchestrator: it routes each turn to a specialized lane, then streams through the one gated pipeline. `linear` is that same pipeline with no routing, one pass, retrieve, ground, answer. Both share the same retrieval and the same guards; only the control flow differs, so the demo can show routing without slowing down the common case.

**Retired:** a third `agent` brain, a LangGraph supervisor with a bounded retry loop, once ran the same turn as a state machine. It was deleted (around 600 lines and two heavyweight dependencies gone). Two brains meant two safety surfaces to keep in parity, the deterministic omni path won on the routing golden set, and the guidance we follow is to add framework abstraction only when a simpler composition fails. `agent` now maps to omni. Scoring the deletion is the point: less code, one safety surface, a stronger story than keeping both.
